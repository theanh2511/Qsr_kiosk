...
function getDetail() {
    ...
    $('#tbl-detail tbody tr').each(function (i, row) {
        ...
        if ($(this).find('.product-unitprice').val().replace(/,/g, '') == '') {
            rowdata['UnitPrice'] = '0';
        } else {
            rowdata['UnitPrice'] = $(this).find('.product-unitprice').val().replace(/,/g, '');
        }
